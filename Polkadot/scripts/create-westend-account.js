...
async function generateNewAccount() {
    log('🔐 Generating new AssetHub Westend account...', 'bright');
    ...
    const accountFile = path.join(__dirname, '../account-info-westend.json');
...
async function checkBalance(api, address) {
    try {
        ...
        logInfo(`Current balance: ${freeBalance.toFixed(4)} WND`);
        return freeBalance;
    } catch (error) {
...
async function attemptFaucetFunding(address) {
    log('\n🚰 Attempting to get funding from AssetHub Westend faucet...', 'bright');
    
    try {
        // Try the web faucet approach
        logInfo('Please visit the AssetHub Westend faucet to fund your account:');
        log(`🌐 https://assethub-westend.subscan.io/`, 'blue');
        log(`📧 Enter your address: ${address}`, 'green');
        
        // Also try Matrix faucet
        logInfo('Alternative: Join Matrix channel for faucet:');
        log(`💬 https://matrix.to/#/#westend-faucet:matrix.org`, 'blue');
...
async function monitorBalance(api, address, initialBalance) {
    ...
    const checkInterval = setInterval(async () => {
        ...
        if (currentBalance > initialBalance) {
            logSuccess(`🎉 Funding received! New balance: ${currentBalance.toFixed(4)} WND`);
...
async function main() {
    try {
        log('🚀 Creating new AssetHub Westend testnet account', 'bright');
        log('==============================================', 'bright');
        
        // Step 1: Generate new account
        const { account, mnemonic } = await generateNewAccount();
        
        // Step 2: Connect to AssetHub Westend
        log('\n🔗 Connecting to AssetHub Westend testnet...', 'bright');
        const provider = new WsProvider('wss://westend-assets-hub-rpc.polkadot.io');
        const api = await ApiPromise.create({ provider });
        
        logSuccess('Connected to AssetHub Westend testnet');
        
        // Step 3: Check initial balance
        const initialBalance = await checkBalance(api, account.address);
        
        if (initialBalance > 0) {
            logSuccess(`Account already has ${initialBalance.toFixed(4)} WND`);
        } else {
        ...
        log('='.repeat(60), 'cyan');
        log(`Address: ${account.address}`, 'green');
        log(`Balance: ${finalBalance.toFixed(4)} WND`, 'green');
        log(`Network: AssetHub Westend Testnet`, 'blue');
        log(`Explorer: https://assethub-westend.subscan.io/account/${account.address}`, 'blue');
