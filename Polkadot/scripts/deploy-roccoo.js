...
// Configuration
const ROCOCO_NODE_URL = 'wss://rococo-rpc.polkadot.io';
...
async function checkRococoConnection() {
    logStep('1', 'Connecting to Rococo testnet...');
    
    try {
        const provider = new WsProvider(ROCOCO_NODE_URL);
        ...
        logError(`Failed to connect to Rococo testnet at ${ROCOCO_NODE_URL}`);
...
async function setupDeploymentAccount() {
    ...
    if (!mnemonic) {
        ...
        logWarning('Make sure your account has enough ROC tokens for deployment');
...
async function checkAccountBalance(api, account) {
    ...
    try {
        ...
        logInfo(`Account balance: ${freeBalance.toFixed(4)} ROC`);
        
        if (freeBalance < 10) {
            logWarning('Low balance detected. You need at least 10 ROC for deployment');
            logInfo('Get ROC tokens from: https://rococo-faucet.polkadot.io/');
        } else {
...
async function saveDeploymentInfo(deployedContracts) {
    ...
    const deploymentInfo = {
        ...
        rpcUrl: ROCOCO_NODE_URL,
        explorer: 'https://rococo.subscan.io/'
    };
    
    const deploymentFile = path.join(__dirname, '../deployment-roccoo.json');
    ...
    log('\n🔗 Useful Links:', 'cyan');
    log(`- Explorer: ${deploymentInfo.explorer}`, 'blue');
    log(`- Polkadot.js Apps: https://polkadot.js.org/apps/?rpc=${encodeURIComponent(ROCOCO_NODE_URL)}`, 'blue');
...
async function main() {
    try {
        log('🚀 Starting Rococo Testnet Deployment', 'bright');
        log('=====================================', 'bright');
        
        // Step 1: Connect to Rococo
        const api = await checkRococoConnection();
